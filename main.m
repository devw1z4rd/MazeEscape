function main
    ...
    % nastaviť parametre podľa obtiažnosti
    if difficulty == 1
        wall_chance = 0.2;
        num_bonuses = 2;
        time_limit = 60;
    ...
    valid_maze = false;
    % generovať platný labyrint
    while ~valid_maze
        maze = ones(maze_size); % inicializovať steny
        
        % aáhodne umiestniť priepustné cesty
        for i = 2:maze_size-1
            for j = 2:maze_size-1
                if rand > wall_chance
                    maze(i,j) = 0;
        ...
        % vytvoriť východné a vstupné cesty
        maze(2:maze_size-1, 2) = 0;
        maze(maze_size-1, 2:maze_size-1) = 0;
        
        maze(2,2) = 2; % start
        maze(maze_size-1,maze_size-1) = 3; % exit
        
        % zistiť prázdne miesta
        empty_count = 0;
        for i = 1:maze_size
            for j = 1:maze_size
                if maze(i,j) == 0
...
% skontrolovať existenciu cesty v labyrinte
function valid = check_path(maze, maze_size)
    ...
    % či exit je dosiahnuteľný
    if visited(maze_size-1, maze_size-1) == 0
        valid = false;
        return;
    end
    
    % či sú dosiahnuteľné všetky bonusy
    valid = true;
    for i = 1:bonus_count
...
% najlepšie skóre z uložených výsledkov
...
% uložiť nové skóre do súboru (maze_scores.txt)
